...
英雄胜率榜使用 OpenDota `/scenarios/laneRoles` 当前周及前三周的预聚合数据（官方约一个月保留窗口），在本地汇总所有位置和比赛时长桶后取胜率前 10，并展示实际起止日期。OpenDota 遇到 429、5xx、522 或网络错误时会进行有限指数退避重试；成功响应会缓存到 `res/monthly_hero_stats_cache.json`。重试仍失败时仅使用 48 小时内的最后成功缓存，并在榜单中标注缓存状态。
...
# 每晚 20:00 接收最近一个月全分段英雄胜率 Top 10 的 QQ 群 openid
...
用户私聊机器人时，消息会直接交给 DeepSeek 生成回复，并通过原私聊会话返回；私聊发送 `高胜率英雄` 会直接返回最近一个月全分段英雄胜率 Top 10。机器人会按群和私聊用户隔离五分钟短期上下文，避免不同会话之间串话。
