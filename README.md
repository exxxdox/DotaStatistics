...
英雄胜率榜使用 OpenDota `scenarios` 当前周及前三周的预聚合数据（官方约一个月保留窗口），汇总所有位置后取胜率前 10，并展示实际起止日期。OpenDota 遇到 429、5xx、522 或网络错误时会进行有限指数退避重试；成功响应会缓存到 `res/monthly_hero_stats_cache.json`。重试仍失败时仅使用 48 小时内的最后成功缓存，并在榜单中标注缓存状态。
